 ...
 * Please check {@link http://www.yiiframework.com/doc/guide/database.ar the
 * Guide} for more details
 * about this class.
 *
 * @property CDbCriteria $dbCriteria The query criteria that is associated with
 *           this model.
 *           This criteria is mainly used by {@link scopes named scope} feature
 *           to accumulate
 *           different criteria specifications.
 * @property CActiveRecordMetaData $metaData The meta for this AR class.
 * @property CDbConnection $dbConnection The database connection used by active
 *           record.
 * @property CDbTableSchema $tableSchema The metadata of the table that this AR
 *           belongs to.
 * @property CDbCommandBuilder $commandBuilder The command builder used by this
 *           AR.
 * @property array $attributes Attribute values indexed by attribute names.
 * @property boolean $isNewRecord Whether the record is new and should be
 *           inserted when calling {@link save}.
 *           This property is automatically set in constructor and {@link
 *           populateRecord}.
 *           Defaults to false, but it will be set to true if the instance is
 *           created using
 *           the new operator.
 * @property mixed $primaryKey The primary key value. An array (column
 *           name=>column value) is returned if the primary key is composite.
 *           If primary key is not defined, null will be returned.
 * @property mixed $oldPrimaryKey The old primary key value. An array (column
 *           name=>column value) is returned if the primary key is composite.
 *           If primary key is not defined, null will be returned.
 ...
 * @since 1.0
 */
abstract class CActiveRecordO extends CActiveRecord {
	 ...
	 *        	scenario name. See {@link CModel::scenario} for more details
	 *        	about this parameter.
	 *        	Note: in order to setup initial model parameters use {@link
	 *        	init()} or {@link afterConstruct()}.
	 *        	Do NOT override the constructor unless it is absolutely
	 *        	necessary!
	 */
	 ...
	 * It changes the query caching parameter of the {@link dbConnection}
	 * instance.
	 *
	 ...
	 * If the relation is HAS_MANY or MANY_MANY, it will return an array of
	 * objects
	 * or an empty array.
	 *
	 * @param string $name
	 ...
	 *        	whether to reload the related objects from database. Defaults
	 *        	to false.
	 *        	If the current record is not a new record and it does not have
	 *        	the related objects loaded they
	 *        	will be retrieved from the database even if this is set to
	 *        	false.
	 *        	If the current record is a new record and this value is false,
	 *        	the related objects will not be
	 *        	retrieved from the database.
	 * @param mixed $params
	 *        	array or CDbCriteria object with additional parameters that
	 *        	customize the query conditions as specified in the relation
	 *        	declaration.
	 *        	If this is supplied the related record(s) will be retrieved
	 *        	from the database regardless of the value or {@link $refresh}.
	 *        	The related record(s) retrieved when this is supplied will
	 *        	only be returned by this method and will not be loaded into
	 *        	the current record's relation.
	 *        	The value of the relation prior to running this method will
	 *        	still be available for the current record if this is supplied.
	 * @return mixed the related object(s).
	 * @throws CDbException if the relation is not specified in {@link
	 *         relations}.
	 */
	 ...
	 * @param string $name
	 *        	the relation name
	 * @return boolean a value indicating whether the named related object(s)
	 *         has been loaded.
	 */
	 ...
	 *        	whether to create a criteria instance if it does not exist.
	 *        	Defaults to true.
	 * @return CDbCriteria the query criteria that is associated with this
	 *         model.
	 *         This criteria is mainly used by {@link scopes named scope}
	 *         feature to accumulate
	 *         different criteria specifications.
	 */
	 ...
	 * Note, default scope only applies to SELECT queries. It is ignored for
	 * INSERT, UPDATE and DELETE queries.
	 * The default implementation simply returns an empty array. You may
	 * override this method
	 * if the model needs to be queried with some default criteria (e.g. only
	 * active records should be returned).
	 *
	 * @return array the query criteria. This will be used as the parameter to
	 *         the constructor
	 *         of {@link CDbCriteria}.
	 */
	...
	public function getMetaData() {
		...
		if (! array_key_exists ( $className, self::$_md )) {
			self::$_md [$className] = null; // preventing recursive invokes of
			                                // {@link
	 ...
	 * By calling this method, this AR class will regenerate the meta data
	 * needed.
	 * This is useful if the table schema has been changed and you want to use
	 * the latest
	 * available table schema. Make sure you have called {@link
	 * CDbSchema::refresh}
	 * before you call this method. Otherwise, old table schema data will still
	 * be used.
	 */
	 ...
	 * You may override this method if the table is not named after this
	 * convention.
	 *
	 ...
	 *
	 * There are four types of relations that may exist between two active
	 * record objects:
	 * <ul>
	 ...
	 * <li>MANY_MANY: e.g. a member has many skills and a skill belongs to a
	 * member.</li>
	 * </ul>
	 *
	 ...
	 * It retrieves the aggregational information about the related objects,
	 * such as the number
	 * of comments for each post, the average rating for each product, etc.
	 *
	 * Each kind of related objects is defined in this method as an array with
	 * the following elements:
	 * <pre>
	 * 'varName'=>array('relationType', 'className', 'foreignKey', ...additional
	 * options)
	 * </pre>
	 * where 'varName' refers to the name of the variable/property that the
	 * related object(s) can
	 * be accessed through; 'relationType' refers to the type of the relation,
	 * which can be one of the
	 * following four constants: self::BELONGS_TO, self::HAS_ONE, self::HAS_MANY
	 * and self::MANY_MANY;
	 * 'className' refers to the name of the active record class that the
	 * related object(s) is of;
	 * and 'foreignKey' states the foreign key that relates the two kinds of
	 * active record.
	 * Note, for composite foreign keys, they can be either listed together,
	 * separated by commas or specified as an array
	 * in format of array('key1','key2'). In case you need to specify custom
	 * PK->FK association you can define it as
	 * array('fk'=>'pk'). For composite keys it will be
	 * array('fk_c1'=>'pk_с1','fk_c2'=>'pk_c2').
	 * For foreign keys used in MANY_MANY relation, the joining table must be
	 * declared as well
	 ...
	 * elements:
	 * <ul>
	 * <li>'select': string|array, a list of columns to be selected. Defaults to
	 * '*', meaning all columns.
	 * Column names should be disambiguated if they appear in an expression
	 * (e.g. COUNT(relationName.name) AS name_count).</li>
	 * <li>'condition': string, the WHERE clause. Defaults to empty. Note,
	 * column references need to
	 * be disambiguated with prefix 'relationName.' (e.g.
	 * relationName.age&gt;20)</li>
	 * <li>'order': string, the ORDER BY clause. Defaults to empty. Note, column
	 * references need to
	 * be disambiguated with prefix 'relationName.' (e.g. relationName.age
	 * DESC)</li>
	 * <li>'with': string|array, a list of child related objects that should be
	 * loaded together with this object.
	 ...
	 * of the array that stores related objects. This option is only available
	 * to
	 * HAS_MANY and MANY_MANY relations.</li>
	 * <li>'scopes': scopes to apply. In case of a single scope can be used like
	 * 'scopes'=>'scopeName',
	 * in case of multiple scopes can be used like
	 * 'scopes'=>array('scopeName1','scopeName2').
	 * This option has been available since version 1.1.9.</li>
	 * </ul>
	 *
	 * The following options are available for certain relations when lazy
	 * loading:
	 * <ul>
	 * <li>'group': string, the GROUP BY clause. Defaults to empty. Note, column
	 * references need to
	 * be disambiguated with prefix 'relationName.' (e.g. relationName.age).
	 * This option only applies to HAS_MANY and MANY_MANY relations.</li>
	 * <li>'having': string, the HAVING clause. Defaults to empty. Note, column
	 * references need to
	 * be disambiguated with prefix 'relationName.' (e.g. relationName.age).
	 * This option only applies to HAS_MANY and MANY_MANY relations.</li>
	 * <li>'limit': limit of the rows to be selected. This option does not apply
	 * to BELONGS_TO relation.</li>
	 * <li>'offset': offset of the rows to be selected. This option does not
	 * apply to BELONGS_TO relation.</li>
	 * <li>'through': name of the model's relation that will be used as a bridge
	 * when getting related data. Can be set only for HAS_ONE and HAS_MANY. This
	 * option has been available since version 1.1.7.</li>
	 * </ul>
	 *
	 ...
	 * </pre>
	 *
	 * @return array list of related object declarations. Defaults to empty
	 *         array.
	 */
	 ...
	 * A named scope represents a query criteria that can be chained together
	 * with
	 * other named scopes and applied to a query. This method should be
	 * overridden
	 ...
	 *
	 * @return array the scope definition. The array keys are scope names; the
	 *         array
	 *         values are the corresponding scope definitions. Each scope
	 ...
	 * This would return all column names of the table associated with this AR
	 * class.
	 *
	 ...
	 * In particular, if the attribute name is in the form of
	 * "post.author.name",
	 * then this method will derive the label from the "author" relation's
	 * "name" attribute.
	 *
	 ...
	 * @param string $name
	 *        	the relation name
	 * @return CActiveRelation the named relation declared for this AR class.
	 *         Null if the relation does not exist.
	 */
	 ...
	 * @param string $name
	 *        	the attribute name
	 * @return mixed the attribute value. Null if the attribute is not set or
	 *         does not exist.
	 * @see hasAttribute
	 */
	 ...
	 * @param mixed $value
	 *        	the attribute value.
	 * @return boolean whether the attribute exists and the assignment is
	 ...
	 *        	If false, it means a HAS_ONE or BELONGS_TO object and no index
	 *        	is needed.
	 */
	 ...
	 *        	If this is true (default), then all attribute values will be
	 *        	returned, including
	 *        	those that are not loaded from DB (null will be returned for
	 *        	those attributes).
	 *        	If this is null, all attributes except those that are not
	 *        	loaded from DB will be returned.
	 ...
	 *
	 * The record is inserted as a row into the database table if its {@link
	 * isNewRecord}
	 * property is true (usually the case when the record is created using the
	 * 'new'
	 * operator). Otherwise, it will be used to update the corresponding row in
	 * the table
	 * (usually the case if the record is obtained using one of those 'find'
	 * methods.)
	 *
	 * Validation will be performed before saving the record. If the validation
	 * fails,
	 * the record will not be saved. You can call {@link getErrors()} to
	 * retrieve the
	 * validation errors.
	 *
	 * If the record is saved via insertion, its {@link isNewRecord} property
	 * will be
	 * set false, and its {@link scenario} property will be set to be 'update'.
	 * And if its primary key is auto-incremental and is not set before
	 * insertion,
	 * the primary key will be populated with the automatically generated key
	 * value.
	 *
	 * @param boolean $runValidation
	 *        	whether to perform validation before saving the record.
	 *        	If the validation fails, the record will not be saved to
	 *        	database.
	 ...
	 *
	 * @return boolean whether the record is new and should be inserted when
	 *         calling {@link save}.
	 *         This property is automatically set in constructor and {@link
	 *         populateRecord}.
	 *         Defaults to false, but it will be set to true if the instance is
	 ...
	 *
	 * @param boolean $value
	 *        	whether the record is new and should be inserted when calling
	 *        	{@link save}.
	 * @see getIsNewRecord
	 */
	 ...
	 * By setting {@link CModelEvent::isValid} to be false, the normal {@link
	 * save()} process will be stopped.
	 *
	 ...
	 * By setting {@link CModelEvent::isValid} to be false, the normal {@link
	 * delete()} process will be stopped.
	 *
	 * @param CModelEvent $event
	 *        	the event parameter
	 */
	 ...
	 * find call, you can use {@link getDbCriteria()} to customize it based on
	 * your needs.
	 * When modifying criteria in beforeFind you have to make sure you are using
	 * the right
	 * table alias which is different on normal find and relational call.
	 * You can use {@link getTableAlias()} to get the alias used for the
	 * upcoming find call.
	 * Please note that modification of criteria is fully supported as of
	 * version 1.1.13.
	 * Earlier versions had some problems with relational context and applying
	 * changes correctly.
	 *
	 * @param CModelEvent $event
	 *        	the event parameter
	 ...
	 * If you want to access or modify the query criteria used for the
	 * count call, you can use {@link getDbCriteria()} to customize it based on
	 * your needs.
	 * When modifying criteria in beforeCount you have to make sure you are
	 * using the right
...
// 	public function action
	 ...
	 * You may override this method to do any preparation work for record
	 * saving.
	...
	protected function beforeSave() {
		if ($this->getIsNewRecord ()){
			...
			if(!$this->$id)
			{
				$this->$id = OracleTools::getNewId ( $this->tableName () ,$id);
			}
...
// 		}
	 ...
	 * You may override this method to do postprocessing after the record is
	 * deleted.
	 * Make sure you call the parent implementation so that the event is raised
	 * properly.
	 */
	 ...
	 * {@link findAllByPk}, {@link findByAttributes}, {@link
	 * findAllByAttributes},
	 ...
	 * If the table's primary key is auto-incremental and is null before
	 * insertion,
	 * it will be populated with the actual value after insertion.
	 * Note, validation is not performed in this method. You may call {@link
	 * validate} to perform the validation.
	 ...
	 *        	list of attributes that need to be saved. Defaults to null,
	 *        	meaning all attributes that are loaded from DB will be saved.
	 * @return boolean whether the attributes are valid and the record is
	 ...
	 * of an existing row dataset and does NOT call either {@link beforeSave} or
	 * {@link afterSave}.
	 * Also note that this method does neither attribute filtering nor
	 * validation.
	 ...
	 *
	 * @param array $attributes
	 *        	attributes to be updated. Each element represents an attribute
	 *        	name
	 *        	or an attribute value indexed by its name. If the latter, the
	 *        	record's
	 ...
	 *
	 * @return boolean whether the row still exists in the database. If true,
	 *         the latest data will be populated to this active record.
	 */
	 ...
	 * The comparison is made by comparing table name and the primary key values
	 * of the two active records.
	 *
	 * @param CActiveRecord $record
	 *        	record to compare to
	 * @return boolean whether the two active records refer to the same row in
	 *         the database table.
	 */
	 ...
	 *
	 * @return mixed the primary key value. An array (column name=>column value)
	 *         is returned if the primary key is composite.
	 *         If primary key is not defined, null will be returned.
	 */
	 ...
	 * After calling this method, the old primary key value can be obtained from
	 * {@link oldPrimaryKey}.
	 *
	 * @param mixed $value
	 *        	the new primary key value. If the primary key is composite,
	 *        	the new value
	 ...
	 * The value remains unchanged even if the primary key attribute is manually
	 * assigned with a different value.
	 *
	 * @return mixed the old primary key value. An array (column name=>column
	 *         value) is returned if the primary key is composite.
	 *         If primary key is not defined, null will be returned.
	 * @since 1.1.0
	 */
	 ...
	 *
	 * @param CDbCriteria $criteria
	 *        	the query criteria. This parameter may be modified by merging
	 *        	{@link dbCriteria}.
	 */
	 ...
	 *        	whether to check if a table alias is defined in the applied
	 *        	scopes so far.
	 *        	This parameter must be set false when calling this method in
	 *        	{@link defaultScope}.
	 ...
	 *
	 * @param string $alias
	 *        	the table alias to be used in queries. The alias should NOT be
	 *        	quoted.
	 * @since 1.1.3
	 */
	 ...
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 * @return CActiveRecord[] list of active records satisfying the specified
	 *         condition. An empty array is returned if none is found.
	 */
	 ...
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 * @return CActiveRecord[] the records found. An empty array is returned if
	 *         none is found.
	 */
	 ...
	 *
	 * @param array $attributes
	 *        	list of attribute values (indexed by attribute names) that the
	 *        	active records should match.
	 *        	An attribute value can be an array which will be used to
	 ...
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 * @return string the number of rows satisfying the specified query
	 *         condition. Note: type is string to keep max. precision.
	 */
	 ...
	 * This is equivalent to calling {@link CDbCommand::queryScalar} with the
	 * specified
	 ...
	 * @param array $params
	 *        	parameters to be bound to the SQL statement
	 * @return string the number of rows using the given SQL statement. Note:
	 ...
	 * are relation names, and the array values are the corresponding query
	 * options.
	 * For example,
	 * <pre>
	 ...
	 * This is only used in relational AR query. Please refer to {@link
	 ...
	 * See {@link find()} for detailed explanation about $condition and $params.
	 * Note, the attributes are not checked for safety and validation is NOT
	 * performed.
	 *
	 * @param mixed $pk
	 *        	primary key value(s). Use array for multiple primary keys. For
	 ...
	 * See {@link find()} for detailed explanation about $condition and $params.
	 * Note, the attributes are not checked for safety and no validation is
	 * done.
	 *
	 * @param array $attributes
	 *        	list of attributes (name=>$value) to be updated
	 ...
	 * Note, this updates all rows of data unless a condition or criteria is
	 * specified.
	 * See {@link find()} for detailed explanation about $condition and $params.
	 *
	 * @param array $counters
	 ...
	 *        	whether to call {@link afterFind} after the record is
	 *        	populated.
	 * @return CActiveRecord the newly created active record. The class of the
	 *         object is the same as the model class.
	 ...
	 * @param boolean $callAfterFind
	 *        	whether to call {@link afterFind} after each record is
	 *        	populated.
	 * @param string $index
	 *        	the name of the attribute whose value will be used as indexes
	 *        	of the query result array.
	 *        	If null, it means the array will be indexed by zero-based
	 *        	integers.
